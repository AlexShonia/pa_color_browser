...
router.route("/").get((req, res) => {
	...
	if (keyword !== "undefined" && keyword) {
		...
		keywordArray = keywordArray.map(function (word) {
			return new RegExp("\\b" + word + "\\b", "i");
		});
		query["$and"].push({
			$or: [
				{ title: { $all: keywordArray } },
				{ tags: { $all: keywordArray } },
			],
		});
	}
	...
	if (height !== "undefined" && height && width !== "undefined" && width) {
		if (sizeType === "min") {
			...
			query["$and"].push({
				$and: [{ height: { $eq: height } }, { width: { $eq: width } }],
			});
		}
	...
	if (query["$and"].length > 0) {
		...
		// Handle case with no filters, e.g., return all images or a message
		Image.find({})
			.skip((page - 1) * 50)
			.limit(50)
			.sort({ faves: -1 })
			.then((images) => {
				res.json(images);
				console.log(Image.modelName);
				console.log(Image.collection.name);
			})
